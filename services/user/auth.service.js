...
const login = async (data) => {
    try {
        const user = await User.findOne({ mobile: data.mobile, otp: data.otp });
...
const checkOtp = async (data) => {
    ...
    if (user) {
        ...
        return ({ status: httpStatus.OK, data: user._id, message: "OTP matched" });
    }
    return ({ status: httpStatus.INTERNAL_SERVER_ERROR, message: "Invalid OTP" });
}
...
const changeForgotPassword = async (data) => {
    ...
    if (await User.findByIdAndUpdate(data.userId, { password: hashPassword }, {
        new: false
    })) {
...
module.exports = {
    register, login, details, socialLogin, forgotPassword, checkOtp, changeForgotPassword, sendloginotp
}
